...
def validate(data_type, model, seq_length=16, saved_model=None,
    ...
    scores=np.zeros([idx])
    total=np.zeros([idx])
    for X,y in data.gen_test('test', data_type,target): 
    ...
    cm = confusion_matrix(true_label, predicted).ravel()
...
def main():
    model = 'r3d_18' 
    target='action'
    features='IV3' # Choose model name (resnet50, IV3,vgg16,vgg19,IR2,resnet152,densenet169,xception,efficientnetb7)
    seq_length=16
    batch_size=2
    ...
    if model=='r3d_18':
        model_file_name='r3d_18-images_action.008-1.665.hdf5'
        image_shape = (224, 224, 3)
        data_type = 'images' 
    if target== 'views':
        if model=='mlp' and features=='IV3':
            model_file_name='IV3_mlp-features_views.023-0.602.hdf5'
            image_shape = None
            data_type = 'features' 
    if target== 'situation':
            if model=='mlp' and features=='IV3':
                model_file_name='V3_mlp-features_situation.011-1.007.hdf5'
                image_shape = None
                data_type = 'features'             
